...
- **1,914+ テスト** — ローカルユニットテスト + GitHub統合テストで品質を担保
...
インストール後、タスクを保存するプライベート GitHub リポジトリを作成し、下記の環境変数を設定してください（手順の3〜4を参照）。
...
**English patterns:** today / tomorrow / day after tomorrow / next week / next month / this weekend / end of this month / end of next month / in N days / in N weeks / in N months / next Monday〜next Sunday

> **Note:** 英語の日付パターン（例: `--due tomorrow`）は言語設定に関係なく常に使用可能です。
...
4. Issue 番号は正の整数のみ許可
5. 日付は `YYYY-MM-DD` または `M/D` 形式のみ許可し、かつ実在するカレンダー上の日付であることを検証（`2026-13-01` や `2/30` は拒否）。`M/D` は常に具体的な年に正規化され、結果が今日より過去になる場合は翌年に繰り上げる
6. recur パターンは `daily` / `weekly` / `monthly` / `weekdays` と、`weekly:<曜日>` / `monthly:<日>` のサフィックス付き形式のみ許可。サフィックスなしの `monthly` は、翌月に対象日が存在しない場合（例: `1/31`）はその月の末日にクランプし（`1/31→2/28`）、以降の月でも元の日には戻らない
...
└── tests/
    ...
    ├── run-tests.sh        # ローカルユニットテスト（1,914+ アサーション。書き込み系を含む総数）
    ...
    └── stubs/
        └── octokit-stub.js # @octokit/rest のスタブ（GitHub APIを叩かずにユニットテストを実行するため）
```
...
基本機能（40+ コマンド）はすべて無料で利用できます。以下は生産性をさらに高めるための追加機能です。
