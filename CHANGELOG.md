...
- `/todo add` で `--recur` を指定し `--due` を省略した場合、初回 `due` を自動算出するようになった（Issue #1950）。`due` が未設定の `routine` Issue は `/todo today` のどのセクションにも表示されないという構造的な欠陥があり（表示ロジックが `due` の有無で分岐するため、`due` が空だとどの分岐にも該当しない）、一度も `done` を打つ機会がないまま存在に気づけなくなる問題があった。新設した `firstDueFromToday()` が recur パターンから「`today` を含む、`today` 以降で最初の該当日」を算出する: `daily` / `weekly`（無サフィックス）/ `monthly`（無サフィックス）は `today` 自身、`weekdays` は土日なら翌月曜、`weekly:<曜日>` / `monthly:<日>` は該当日が `today` 以降に来るまで進める（該当日がその月に存在しない場合は月末にクランプし、既存の `next-due` と同じ警告を出す）。**`--due` を明示指定した場合は上書きしない**。GTDカテゴリでは分岐しないため、`routine` 以外（`waiting` 等）で `recur` を使う場合にも同様に働く
...
- `runList()` の `extra` 走査ループから `@` 始まりトークンの判定分岐を削除した（Issue #1934 パート3）。**これは挙動の変わらないデッドコード削除である**。`parseArgs()` が `@` 始まりトークンを無条件に `parsed.contexts` へ消費するため、この分岐は構造的に到達不能だった（実測で確認）。一方、当初 Issue が同じくデッドコードと記述していた `#` 始まり分岐は、`parseArgs()` の `#tag` 消費条件に「空白を含まない」という制約があるため、**空白を含む `#` トークン**（例: `list "#1299 タイトルの続き"`）が到達しうることが判明し、削除しなかった（削除するとこの入力が「エラー終了」から「黙って無視」に変わってしまう。挙動変更のためスコープ外）。回帰テストとして `tests/run-tests-write.sh` に §W30（4ケース）を新設した。全1869件PASS（書き込み系922/922）
...
- `来週` / `来月` / `今週末` の `--due` 日付表現の説明が、実装の挙動と食い違っていたのを修正した（Issue #2009。#2004 で判明した「記載どおりの意図と違う挙動を選んでしまう」型の誤記が、`来週`/`来月` は記載そのものが実装と異なるためより起きやすいと判断し着手）。`normalizeDue()`（`todo-engine.js:973-975`）を基準日を変えて実測したところ、`来週` は `add(d(), 7)`（単純な+7日。曜日は変わらない）、`来月` は `addMonths(d(), 1)`（同じ日を翌月へ）であり、「月曜」「1日」へ丸める処理は存在しなかった。一方 `todo.md` / `docs/todo-manual.md` / `README.md` は「来週月曜」「来週の月曜日」「来月1日」と記載していた（`README.ja.md` の該当2行は元々実装どおり正しく、修正不要）。記載を実装に合わせる方針（実装変更は既存Issueの期日設定の後方互換を壊すため見送り）で、`来週`→「+7日（同じ曜日）」、`来月`→「翌月の同じ日」へ表現を統一した。あわせて `今週末`（`dow===6?0:6-dow` で土曜実行時は当日を返す）の説明「次の土曜日」も、未来を含意し誤読を招くため「今週の土曜日（土曜なら当日）」へ訂正した（`docs/todo-manual.md` / `README.ja.md` / 英語版 `README.md` の `this weekend` = "Saturday of this week (today, if today is Saturday)"）。実装（`todo-engine.js`）自体の変更はない。修正対象は `todo.md`（SSoT・production）/ `docs/todo-manual.md` / `README.md` / `README.ja.md` の4ファイル
...
- `parseArgs()` が消費する17種の値付きフラグ（`--due` / `--desc` / `--recur` / `--project` / `--priority` / `--estimate` / `--actual` / `--due-offset` / `--color` / `--activate` / `--before` / `--depends-on` / `--resume-condition` / `--note` / `--body` / `--body-file` / `--label`）のうち、各ハンドラが実際には読まないフラグが、エラーも警告も出さずに値だけ消えていた不具合を修正した（Issue #1934 パート1）。#1921 で導入した `findUnknownFlag()`/`guardUnknownFlag()` は `parsed.extra`（parseArgs が解釈できなかったトークン）だけを検査対象にしており、「フラグの綴りは正しく `parseArgs()` が正常に消費したが、そのハンドラが読まない」という第3の型（`add next "テスト" --note "メモ"` の `--note` は投稿されない、`add ... --actual` / `--color` / `--due-offset` も同様に無視される、`edit 42 --note` も効かない、`list --due 2026-09-10` はフィルタが効かず全件表示される）は検出できなかった。新設した `FLAG_FIELD_MAP`（フィールド名→代表フラグ表記）・`FLAG_SUPPORTED_BY`（フィールド名→実際に読むコマンド一覧）・`findUnsupportedFlagField()`（純粋関数）・`guardUnsupportedFlag()` により、`parseArgs()` を呼ぶ8ハンドラ（`add` / `list` / `done` / `move` / `edit` / `label add` / `template save`のインライン形式 / `bulk done`）それぞれに「実際に読むフィールド一覧」を明示し、それ以外の17フラグ由来フィールドが値ありで渡された場合は Usage・エラー本文（`error.flag_not_supported`、ja/en）・「どのコマンドでは使えるか」のヒント（`error.flag_not_supported_hint`）を出して `exit 1` するようにした。既存の `error.unknown_flag`（タイポ用）とはメッセージキーを分離しており、`--note` が `add` では「不明」ではなく「対象外」であることを区別して伝える。判定は各ハンドラの `guardUnknownFlag(...)` 呼び出しの直後（API 副作用より前）に置いたため、サポート対象フラグとの混在時（例: `edit 42 --due 2026-09-10 --note "x"`）でも部分適用は起きず、エラー終了時は `issues.update` 等の API 呼び出しが0回であることを確認済み。grep による既存呼び出しの全数調査では、本変更により後方互換が壊れる実呼び出しは1件も見つからなかった（ヒットしたのは症状を説明するドキュメント記述のみ）。回帰テストとして `tests/run-tests.sh` に判定器の単体テスト §52（15アサーション）、`tests/run-tests-write.sh` に8ハンドラの振る舞いテスト §W29（64アサーション）を新設した。追加した8箇所のガードを一時的に無効化して実行し、異常系14ケース（W29-1〜11・W29-13〜15）が全件確実に FAIL することも実測で確認済み。全1826件PASS（書き込み系883/883）。GitHub への実書き込みは行わず、スタブ経由のみで検証した。`todo.md` の共通注記と `help.unknown_flag_note`（ja/en）も、この17フラグに限定した形で「黙って捨てられる」から「エラーになる」へ更新した（`@ctx`/`#tag` が読まれない同型の問題は本パートのスコープ外として残る）
- 上記と同じ構造の問題が `@ctx` / `#tag`（`parseArgs()` が消費する位置トークン）にも残っていたのを塞いだ（Issue #1934 パート1.5）。`@ctx`/`#tag` はフラグではなく位置トークンだが、`parseArgs()` が消費した後にハンドラが読まないと値が黙って消える構造は17フラグと同一で、`done` / `move` / `edit` / `label add` / `bulk done` はいずれも `parsed.contexts`/`parsed.tags` を一切読んでいなかった（`add` / `list` は元から両方読んでいる）。`FLAG_FIELD_MAP`/`FLAG_SUPPORTED_BY` に `contexts`（代表表記 `@ctx`）/`tags`（代表表記 `#tag`）を追加し、`guardUnsupportedFlag()` の対象を17フラグから19フィールドへ拡張した。実装コードを直接確認したところ、`template save`（インライン形式）は `parsed.contexts` は読む（`@ctx` は使える）が `parsed.tags` はこの分岐のどこからも読んでいない非対称が既存コードに存在した（`template save tmpl next #tag` は従来 exit 0 で `#tag` が黙って消えていた）。この非対称を tags 側の実装を直して解消するかはスコープ外の判断とし、今回は `contexts` のみを対象に加えて `#tag` は引き続き「使えません」エラーとした（tags を読むよう実装を変えると挙動追加になるため）。呼び出し元リポジトリ全体の grep 全数調査では、本変更により後方互換が壊れる実呼び出しは1件も見つからなかった。回帰テストとして `tests/run-tests.sh` §52 に4アサーション、`tests/run-tests-write.sh` §W29 パート1.5 に12ケース（異常系8・境界値1・i18n1・正常系3、37アサーション）を新設した。追加したガード対象拡張（`FLAG_FIELD_MAP` の `contexts`/`tags` 追加）を一時的に無効化して実行し、異常系ケース全件が確実に FAIL することも実測で確認済み（正常系の回帰確認ケースと、`assert_no_japanese` 1件のみ既知の理由でPASSしたまま残る。詳細はパート1の同型の注記を参照）。全1863件PASS（書き込み系916/916）。GitHub への実書き込みは行わず、スタブ経由のみで検証した。`todo.md` の共通注記・`help.unknown_flag_note`（ja/en）も、`@ctx`/`#tag` を対象に含めた形へ更新した（`template save` の非対称も明記）
- `due` / `recur` / `link` / `priority` が値の直後に残った非フラグの余剰位置引数（クォート漏れ・値の指定過多）を黙って捨てていた不具合を修正した（Issue #1934 パート2）。`tokens[0]=番号 / tokens[1]=値` の固定インデックスで読むこれら4コマンドは、#1921 パターンCで配線した `guardUnknownFlag()` がフラグ字面のトークンのみを検査していたため、`due 42 2026-09-10 余剰トークン`（`余剰トークン` が黙って捨てられ期日は `2026-09-10` になる）、`link 42 100 101`（project番号は `100` のみ登録される）、`priority 42 p1 p2`（優先度は `p1` のみ設定される）のような余剰は素通りしていた。この挙動は #1921 で「手順書に日本語日付の未クォート運用が実在するため塞がない」という理由で意図的に残していたが、#1930（コミット `3973ff99`）で手順書側のクォート漏れが解消されたため前提が満たされ、本パートで着手した。新設した `guardExtraPositional(extraTokens, usage, hintKey, example)` により、値の後に1つでも非フラグトークンが残っていれば Usage・エラー本文（`error.extra_positional`）・修正例のヒント（`due`/`recur` は値全体をクォートする例、`link`/`priority` は余剰を外す例）を出して `exit 1` する。Issue本文が明示していなかった `recur` も完全に同型のため対象に含めた。`runUnlink`（`tokens.includes('--force')` で走査するだけの構造）は「第2の位置引数スロット」を持たないため本パートの対象外とした（`guardUnknownFlag` 自体が未配線という別の既存ギャップが残っており、別Issue化を検討中）。既存の後方互換仕様固定テスト（`due 42 2026-09-10 余剰トークン` が exit 0 で通ることを固定していたテスト）は本パートで意図的に反転させ `exit 非0` を固定し直した。回帰テストとして `tests/run-tests-write.sh` に §W31（13ケース）を新設し、追加したガード呼び出し4箇所を一時的に無効化して異常系全件が確実に FAIL することも実測で確認済み。全1914件PASS（書き込み系967/967）。GitHub への実書き込みは行わず、スタブ経由のみで検証した。`todo.md` の共通注記・`help.unknown_flag_note`（ja/en）も、この挙動を反映した形へ更新した
- `search` / `archive search` がキーワードを `join(' ')` で組み立てる際、`--json`（`search` のみ）以外のフラグ字面（タイプミス）を検査せず黙って検索クエリの一部に混入させていた不具合を修正した（Issue #1934 パート4）。`add` / `rename` / `template use` 等の同型ハンドラには #1921 で `guardUnknownFlag()` が配線済みだったが、`search` / `archive search` は対象から漏れていた。読み取り専用で副作用がないため実害は小さいが（Issue本文にも明記）、一貫性のため他のハンドラと同じ `guardUnknownFlag()` を配線した。`search --keywrd foo` のようなタイプミスは、従来は文字列 `"--keywrd foo"` のままゼロ件検索されエラーなく完了していたが、修正後は Usage・エラー本文を出して `exit 1` する。回帰テストとして `tests/run-tests-write.sh` に §W32（5ケース）を新設した。全1914件PASS（書き込み系967/967）。GitHub への実書き込みは行わず、スタブ経由のみで検証した。**Issue #1934 は本パートで全パート完了**
- `/todo activate <#> <date>`（`edit <#> --activate <date>` の簡略記法）と `/todo unlink <#> [--force]` の2箇所に残っていた「引数の静かな消失」を修正した（Issue #1937。#1921/#1934 の網から構造的に漏れた残り2件）。(1) `activate` は dispatcher の `case 'activate'` が `rest[0]`/`rest[1]` だけを取り出して `[num,'--activate',date]` という新しい配列を合成してから `runEdit()` へ渡す構造のため、`rest.slice(2)` 以降（フラグ字面・非フラグの余剰トークンとも）が `runEdit` に一切渡らず消滅していた。`due`/`recur` と同様に `guardUnknownFlag()` → `guardExtraPositional()` を `runEdit` への委譲より前に dispatcher 側で直接呼ぶよう修正した（`activate` の値は `normalizeDue()` 経由で自然文の日付を許すため、`due`/`recur` と同じ `hint_quote` を使う）。修正前は `activate 42 2026-09-10 余剰トークン` も `activate 42 2026-09-10 --note "x"` も exit 0 で出力に痕跡なく消えていた。(2) `runUnlink` の `--force` は `tokens.includes('--force')` という走査型判定のみで未知フラグ（タイポ）を検出しておらず、`unlink 42 --forse` は `force=false` のまま処理が進み、たまたま「未登録」エラー（`error.unlink_mismatch`）で止まっていた（デフォルト値 `false` がフェイルセーフに働くため実害は限定的だが、ユーザーは自分の入力ミスに気づけない）。`unlink` は `due`/`recur`/`link`/`priority` と違い「第2の位置引数スロット」を持たないため `guardExtraPositional()` の対象ではなく、`guardUnknownFlag(tokens.slice(1), ['--force'], ...)` のみを配線した。回帰テストとして `tests/run-tests-write.sh` に §W33（23ケース）を新設し、追加したガード呼び出し3箇所（`activate` に2箇所・`unlink` に1箇所）を一時的に無効化して実行し、メッセージ内容・API呼び出し抑止を検証するアサーション群が確実に FAIL することも実測で確認済み（`exit非0` のみを見るアサーション・`assert_no_japanese` の一部は、ガード無効化時に別経路のエラー（スタブ未設定応答等）で偶然 exit 非0 のまま残るため、この2種は単独では判定力を持たない。メッセージ内容とAPI呼び出し回数を見るアサーションが実際の判定を担っている）。全1947件PASS（書き込み系1000/1000）。GitHub への実書き込みは行わず、スタブ経由のみで検証した
- `runMigrateSubIssue` / `runPromoteProject` / `runView save` / `runShow` / `runReviewSomeday` / `runReport` に残っていた「引数の静かな消失」5件を修正した（Issue #1938。#1937 パート3の横断調査で発見された残存候補）。1件ずつ判定器を選定した。(1) `migrate sub-issue` の `--dry-run` は `tokens.includes()` の走査型判定のみで未知フラグ（タイポ）を検出しておらず、`--dryrun`（ハイフン抜け）は `dryRun=false` のまま処理が進み、**ドライランのつもりで実行したユーザーに対し `addSubIssue()` の一括登録が実際に走っていた**（`unlink --force` と違いデフォルト値が危険側で実害が大きい）。`guardUnknownFlag(tokens, ['--dry-run'], ...)` のみを配線した（位置引数を取らないコマンドのため `guardExtraPositional` は対象外）。(2) `promote-project` の `--outcome` は `tokens.indexOf()` の走査型判定のみで、タイポ時はタイトル変更が黙ってスキップされていた。加えて値のクォート漏れ（`--outcome New Marketing Campaign` のような複数トークン）は先頭語だけが使われ残りが黙って消えていた（`due`/`activate` と同型）。`guardUnknownFlag(tokens.slice(1), ['--outcome'], ...)` と `guardExtraPositional()` の両方を配線した（#1938 以前は `--outcome` の end-to-end テストが1件も存在しなかった）。(3) `view save` は GTD ラベル・優先度・`@ctx` のいずれにも一致しないトークンを黙って無視していた（ローカル `views.json` のみの書き込みで GitHub API 影響なし）。フラグ字面（`--nxt` 等）は `guardUnknownFlag`、それ以外の非対応語（`waitng` 等）は `guardExtraPositional` で塞いだ。(4) `show` の `--json` はタイポ（`--jsn`）で通常表示に化けて出力を見ても気づけず、`review-someday` は `tokens[0]` 以降を無条件に無視していた。いずれも「第2の位置引数スロット」を持たないため `guardUnknownFlag` のみを配線した（#1937 の `runUnlink` と同型判断）。(5) `report` は `weekly`/`monthly`/`Nd` のいずれにも一致しない `sub`（例: `monthy`）を `days=7`（weekly相当）へ黙ってフォールバックしており、**「30日のつもりが7日になる」ことに出力からは気づけなかった**。これは他4件のようなフラグのタイポではなく位置引数の値そのものが不正なパターンのため、`guardUnknownFlag`/`guardExtraPositional` は使わず、`sub` が既知の3パターンいずれにも一致しない場合に Usage・エラー本文（新設 `error.report_invalid_sub`、ja/en）を出して `exit 1` する専用分岐を追加した。リポジトリ全体の grep 全数調査（スケジューラ登録を含む）では、本変更により後方互換が壊れる実呼び出しは1件も見つからなかった（同梱ドキュメント中のコマンド例 `promote-project <N> --outcome "タイトル"` / `report 1d` / `migrate sub-issue --dry-run` / `review-someday <N>` はいずれも本変更後も通る形式）。回帰テストとして `tests/run-tests-write.sh` に §W34（63アサーション）を新設し、追加した5箇所のガード・分岐を一時的に無効化して実行し、`migrate sub-issue --dryrun` が実際に `fetchAllOpen` まで到達する／`promote-project` のタイポとクォート漏れがいずれも黙って通る／`view save` の非対応トークンが黙って保存される／`show --jsn` が通常表示になる／`review-someday` のタイポが黙って `reviewedAt` を更新する／`report monthy` が `days=7` の週次レポートを黙って出力する、という5パターンをそれぞれ実測で確認した（詳細は完了報告を参照）。全2010件PASS（書き込み系1063/1063）。GitHub への実書き込みは行わず、スタブ経由のみで検証した。`todo.md` の共通注記に本パートの5件を追記した
- `template save`（インライン形式）が `@ctx` は保存するのに `#tag` は保存できない非対称を修正した（Issue #1936。#1934 パート1.5で「使えませんエラー」に変わっただけで機能ギャップとして残っていた）。テンプレートDBのスキーマに `tags`（`context` と同形式の配列）を追加し、`runTemplate` の save（インライン形式）・save-from（既存Issueからのコピー）・use（テンプレートからのIssue作成）の全経路で `#tag` を `@ctx` と対称に扱うようにした。save-from はコピー元Issueのラベルから `issueToJsonObj()`/`renderIssueList()` と同じ「GTD・project・context・`@claude`・priority 以外」の線引きでタグを抽出する。use はテンプレートの `tags` を Issue 作成時に `runAdd` と同じ色（`0075CA`）・説明でラベル付与する。`tags` キーを持たない既存テンプレートは `template show`/`list`/`use` のいずれでも空配列として扱われ後方互換を保つ。`FLAG_SUPPORTED_BY.tags` を `['add','list']` から `['add','list','template save']` に更新した。回帰テストとして `tests/run-tests-write.sh` に §W35（9ケース・32アサーション）を新設し、`#tag` がエラーになることを固定していた旧テスト（§W29-32）は保存されることを確認する内容へ更新した。追加した実装（`t.tags` 代入・`validateTag` ループ）を一時的に無効化して実行し、該当ケースが確実に FAIL することも実測で確認済み。全2,042件PASS（書き込み系1,095/1,095）。GitHub への実書き込みは行わず、スタブ経由のみで検証した
- `/todo add` のタイトル先頭語が角括弧・鉤括弧・隅付き括弧で装飾された GTD ラベル（`[inbox]` / `「next」` / `【project】` 等）と完全一致する場合、カテゴリ指定の書き損じとしてエラー終了するガードを追加した（Issue #1928。2026-08-09 の実事故 #1757 の再発防止）。事故の構造: `GTD_LABELS.includes(tokens[0])` の完全一致判定は `[inbox]` に一致しないためカテゴリ抽出されず、`[inbox]` はタイトルの先頭語としてそのまま連結され、`[inbox] 本来のタイトル` という汚染タイトルの Issue が exit 0 で作成されていた（ある呼び出し元で「zshのグロブ展開エラー回避」のため `add [inbox]`（裸トークン）を `add "[inbox]"`（クォート済み）へ変更したことで顕在化）。新設した `BRACKETED_GTD_RE`（`GTD_LABELS`/`PROJECT_LABEL` から動的に組み立てる。設計書 `2026-09-01_todo-arg-parsing-pattern-a-1921.md` の元の正規表現は開き・閉じ括弧をともに省略可にしていたが、それだと素の `inbox` 等の単語1語だけの正当なタイトルまで誤検知するため、実装時に両側必須へ変更した）と純粋関数 `findBracketedGtdLabel()`・ラッパ `guardBracketedGtdLabel()` により、タイトル構成トークン列（`parseArgs()` 後の `extra`）の**先頭語のみ**を判定する。先頭語のみを見るのは、実事故が `add "[inbox]" "本来のタイトル"` という2トークン入力（`[inbox]` が単独トークンとしてタイトル先頭語に連結される形）だったため（設計書の「誤検知の範囲」記述はタイトル全体が1語だけの場合に限定するとも読める書き方だったが、実装時にこの実事故のパターンを再現できる形へ判断して先頭語判定を採用した）。クォート済み1トークンに埋め込まれた場合（例: `add next "[inbox] の仕様を調べる"`）は空白を含む1トークンで完全一致に該当しないため通る。検査は #1921/#1934 の未知フラグ・非対応フラグ検査（`guardUnknownFlag`/`guardUnsupportedFlag`）より後、GitHub API 副作用（`ensureLabel`/`issues.create`）より前に置いた。**品質ゲートのレビューで2件の🔴修正必須が見つかり、マージ前に修正した**: (1) `findBracketedGtdLabel()` が `titleTokens[0]` の生の値をトリムせずに検査していたため、前後に空白1文字（半角・タブ・全角スペース）を含めるだけでガードを回避でき、`add ' [inbox] ' '本来のタイトル'` が exit 0 で通ってしまっていた（本Issueが塞ごうとした事故と同型）。判定にのみ `trim()` を使うよう修正し、実際のタイトル文字列は書き換えない。(2) エラーヒント「前後に語を足すか」は、判定が先頭語のみを見る仕様のため実際には「前に足す」場合しか機能せず、後ろに足しても解消しなかった。クォートを主案内にし、語を足す場合は「このラベルより前」に限定する文言へ ja/en/`todo.md` を修正した。回帰テストとして `tests/run-tests-write.sh` に §W36（20ケース・52アサーション）を新設し、追加したガード呼び出し1箇所（`trim()` 込み）を一時的に無効化して実行し、核心アサーション（W36-1/4/5/6/7/14〜17）が確実に FAIL することも実測で確認済み。タブケースは `validateTitle()` の制御文字禁止にも該当し「別の理由」で偽陽性のPASSが起きうることを実装中に発見し、エラー本文の内容確認を追加して判定力を持たせた。全2,094件PASS（書き込み系1,147/1,147）。GitHub への実書き込みは行わず、スタブ経由のみで検証した
...
- `routine` カテゴリで `--recur` を指定せずに `/todo add` した場合、`due` / `recur` がともに空の Issue がノーガードで作成でき、上記の「`/todo today` から消える」欠陥（Issue #1950）をそのまま再現できてしまう不具合を修正した。`routine` は `recur` とセットで使う設計だったが、これを強制するバリデーションが存在しなかった。`/todo add routine <title>`（`--recur` 省略時にエラー）・`/todo move <#> routine`（移動先Issueが既に `recur` を持つことを要求）・`/todo bulk move <#>... routine`（対象ごとに個別判定。`recur` を持たない項目のみエラー表示し、残りは処理を続ける既存の `bulk` の挙動を踏襲）の3経路すべてに同じ不変条件（`routine` へ入るには `recur` が必須）を追加した。生の JSON を直接投入する `api create-issue` 等のエスケープハッチはこの検証を経由しない既存の設計のまま変更していない。エラーメッセージは `--recur weekly` 等の具体例、または既存Issueの場合は `edit --recur` で先に設定してから移動する案内を示す（ja/en 両対応）。**既知の残課題（本修正では対応していない）**: (1) `template save`（`routine` カテゴリで `--recur` 省略）→ `template use` の経路は `add`/`move`/`bulk move` のいずれのガードも経由しない独立実装のため、依然として `due`/`recur` ともに空の `routine` Issue を作成できる。(2) `/todo add` → `/todo edit --recur weekly` → `/todo move <#> routine` という3手順では、`move` の時点で既に `recur` が存在するためガードを通過し、`due` は空のまま残る（`edit` 単体では `due` を自動算出しない設計のため）。いずれも既存挙動を壊さない範囲での意図的なスコープ外判断であり、別途対応を検討する
- `template use <name> [タイトル上書き]` の上書きタイトルが、改行等の制御文字を禁止するタイトルバリデーションを通っていなかった不具合を修正した（Issue #1977）。タイトルを設定・変更する経路（`add` / `rename` / `promote-project --outcome`）はいずれもこのバリデーションを通るが、`template use` は独立実装（`add` を経由せず自前で Issue を作成する）のためこの検証が構造的に効かず、上書きタイトルだけが素通りしていた。上書きタイトル省略時はテンプレート側のタイトルを使う仕様のため、検証は上書きタイトルが指定された場合のみ行う（空文字は従来どおりエラーにしない）。全数調査の結果、他に同型の未検証経路は見つからなかった。回帰テストとして `tests/run-tests-write.sh` の §W28 に2ケースを追加した
